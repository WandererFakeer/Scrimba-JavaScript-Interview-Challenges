...
function render() {
  ...
  for (const [index, detail] of state.podcastData.entries()) {
    const li = document.createElement("li");
    li.textContent = `${index + 1}. ${detail.title}, ${detail.duration} minutes`;
...
function sortByDuration(data, flightLength) {
  state.podcastData =
    flightLength <= 60
      ? data.toSorted((first, second) => first.duration - second.duration)
      : data.toSorted((first, second) => second.duration - first.duration);
